...
import 'package:manager_mqtt/feature/device/device_real_time_controller.dart';
import 'package:manager_mqtt/feature/device/dto/data_device_dto.dart';
import 'package:provider/provider.dart';

class DeviceRealTimeScreen extends StatefulWidget {
  const DeviceRealTimeScreen({super.key});

  @override
  State<DeviceRealTimeScreen> createState() => _DeviceRealTimeScreenState();
}

class _DeviceRealTimeScreenState extends State<DeviceRealTimeScreen> {

  final controller = DeviceRealTimeController();
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((callback) async {
      ...
      if (args is Device) {
        device = args;
        await controller.setDevice(device);
      }
    });
  }
  ...
  @override
  void dispose() {
    controller.stopServiceMqtt();
    super.dispose();
  }

  @override
  Widget build(BuildContext context) {
    return ChangeNotifierProvider<DeviceRealTimeController>.value(
      value: controller,
      child: Consumer<DeviceRealTimeController>(
        builder: (_, control, __) {
          if(control.dataByType.entries.isEmpty) {
            return Scaffold(
              appBar: AppBar(
                title: Text("Información en tiempo real"),
              ...
              body: SingleChildScrollView(
                ...
                child: Column(
                  children: control.dataByType.entries.map((entry) {
                    ...
                    final Map<String, List<DataDeviceDto>> entradas = entry
                        .value;
                    ...
                    return Column(
                      ...
                      children: [
                        Text(nameEntry, style: const TextStyle(
